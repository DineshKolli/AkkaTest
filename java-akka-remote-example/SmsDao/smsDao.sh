...
then
    ...
    java -jar -Dmy-dispatcher.executor="fork-join-executor" -Dmy-dispatcher.fork-join-executor.parallelism-min=$MIN_POOL_SIZE -Dmy-dispatcher.fork-join-executor.parallelism-factor="2.0" -Dmy-dispatcher.fork-join-executor.parallelism-max=$MAX_POOL_SIZE -Dakka.remote.netty.tcp.hostname=$LOCAL_IP -Dakka.loglevel=$LOG_LEVEL  -Dakka.cluster.seed-nodes.0="akka.tcp://SmsDaoCluster@$SEED_NODE_IP:$SEED_NODE_PORT" -Ddatabase.slick-postgres.db.numThreads=$JDBC_THREADS -Ddatabase.slick-postgres.db.properties.url="jdbc:postgresql://$POSTGRES_IP:$POSTGRES_PORT/testdb" SmsDao-1.0-SNAPSHOT-allinone.jar ACTOR_COUNT $ACTOR_COUNT TEST_MODE 0 DISPATCHER "my-dispatcher" "$MODE $2 $3
